<?php
	...
	class RevSliderOutput{
		...
		public $jsToBody = false;
		public $noConflictMode = false;
		...
		private function putFullWidthVideoLayer($videoData){
			...
			if($videoData["type"] == "youtube"):	//youtube
				?>	<div class="tp-caption fade fullscreenvideo" data-nextslideatend="<?php echo $nextslide?>" data-autoplay="<?php echo $autoplay?>"<?php echo $autoplayonlyfirsttime; ?> <?php echo $htmlParams?><?php echo $mute; ?>><iframe src="<?php echo $setBase; ?>www.youtube.com/embed/<?php echo $videoID?>?html5=1&amp;hd=1&amp;wmode=opaque&amp;controls=1&amp;showinfo=0;rel=0;" width="100%" height="100%"></iframe></div><?php 
			else:									//vimeo
		...
		private function putSlides($doWrapFromTemplate){
			...
			if(count($slides) == 1 && $this->oneSlideMode == false){
				//duplicate the slide only if it's a preview mode
		...
		private function putJS(){
	 	  ...
	 	  //modify navigation type (backward compatability)
			...
			<script type="text/javascript">
				...
				var tpj=jQuery;				
				<?php if($this->noConflictMode == true):?>tpj.noConflict();<?php endif;?>
				   ...
				   revapi<?php echo $sliderID?> = tpj('#<?php echo $this->sliderHtmlID?>').show().revolution(
					{
						...
						<?php if($hideThumbsOnMobile == 'off'){ ?>hideNavDelayOnMobile:<?php echo $hideThumbsDelayMobile; ?>,<?php } ?>
		...
		public function putSliderBase($sliderID){
			...
			try{
				...
				if($this->jsToBody == true){
					$urlIncludeJS = GlobalsRevSlider::$url_item_plugin."js/jquery.themepunch.plugins.min.js?rev=". GlobalsRevSlider::SLIDER_REVISION;
