<?php
	...
	class UniteFunctionsRev{
		...
		/**
		 * clear slashes from all array items
		 */
		public static function clearSlashesArray($arr){
			if(!is_array($arr))
				return($arr);
		...
		/**
		 *	encode json, strip slashes from array first.
		 */
		public static function jsonEncodeNoSlashes($arr){
			$arr = self::clearSlashesArray($arr);
			$json = json_encode($arr);
			return($json);
		}
